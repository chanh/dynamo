...
Use `longest-cache-reuse` to select the eligible worker with the greatest request-specific device-cache overlap. The policy does not trade cached blocks for lower load. It uses modeled active load only when multiple workers expose the same device-cache overlap, then uses worker ID to make an exact tie deterministic.

The policy reads:

- Device-resident overlap blocks from `WorkerInputs::CACHE`.
